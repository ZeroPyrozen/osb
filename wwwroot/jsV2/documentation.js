...
switch (lang) {
    case 'Русский':
        url = 'https://api.github.com/repos/Maks1mio/osb-laboratory/contents/RU/Laboratory?ref=main';
        break;
    default:
        url = 'https://api.github.com/repos/Maks1mio/osb-laboratory/contents/Laboratory?ref=main';
}
...
fetch(url)
    ...
    .then(data => {
        for (let key in data) {
            ...
            let match = nameId.match(/^\d+(\.\d+)?/);
            if (match) {
                nameId = `theme-${match[0]}`;
            } else {
                nameId = '';
            }

            let themePath = document.createElement("div");
            themePath.className = "theme-path";
            ...
            fetch(url)
                ...
                .then(url => {
                    for (let urlKey in url) {
                        let subId = url[urlKey].name;
                        let subName = url[urlKey].name;
                        let urlLink = url[urlKey].url;

                        subName = subName.replace(/^\d+(\.\d+)*\s*/, '');
                        subName = subName.replace(/^\.\s*/, '');

                        let match = subId.match(/^\d+(\.\d+)?/);
                        if (match) {
                            subId = match[0];
                        } else {
                            subId = '';
                        ...
                        fetch(urlLink)
                            ...
                            .then(urlLink => {
                                ...
                                console.log(urlLink)
                                for (let subThemeKey in urlLink) {
                                    let itemURL = urlLink[subThemeKey].name;
                                    let itemName = urlLink[subThemeKey].name;
                                    let itemPath = urlLink[subThemeKey].html_url;
                                    let itemLink = urlLink[subThemeKey].download_url;

                                    itemName = itemName.replace(/^\d+(\.\d+)*\s*/, '');
                                    itemName = itemName.replace(/\.md$/, '');

                                    let subTheme = document.createElement("button");
                                    subTheme.id = "test";
                                    subTheme.className = "subtheme";

                                    let img = document.createElement("img");
                                    img.src = "lib/svg/chevron_right.svg";
                                    img.alt = "";
                                    ...
                                    itemURL = `${subId}-${itemURL}`;

                                    subTheme.setAttribute('data-id', itemURL);
                                        ...
                                        if (itemLink) {
                                            ...
                                            fetch(itemLink)
                                                ...
                                                .then(text => {
                                                    ...
                                                    document.getElementById("content").innerHTML += "<a class='edit' href=" + `${itemPath = itemPath.replace("blob", "edit")}` + ">Edit</a>";
                                                    changeButton(subTheme, activeButtons);
                                                    copyFunc();
                                                });
                                        }
                                        window.location.hash = `#${itemURL}`;
                                    });
                                    subThemeList.appendChild(subTheme);
                                }
                                buttonList.appendChild(subThemeList);
                            })
                    }
                })
...
function hashFunc() {
    timer = setInterval(() => {
        if (!hash) {
            console.log("hash");
            const button = document.querySelector(`button[data-id="theme-0-sub-list-1-0.1"]`);
            if (button) {
...
function loading() {
    ...
    button.style.animationName = "loadAnim-off";
...
function changeButton(subTheme, activeButtons) {
    for (let button of activeButtons) {
        button.classList.remove("active");
        clearInterval(timer);
    }
    subTheme.classList.add("active");
}
...
function copyText(inputId) {
    let code = document.getElementById(inputId);
    ...
    if (navigator.clipboard) {
        ...
        // Fallback to using document.execCommand('copy')
...
const LDashboard = document.getElementById("LDashboard");
const RDashboard = document.getElementById("RDashboard");
let menu = 0;

menuButton.addEventListener("click", () => {
    if (menu === 0) {
        LDashboard.style.left = "0px";
        RDashboard.style.left = "350px";
        menu = 1;
    } else if (menu === 1) {
        LDashboard.style.left = "-350px";
        RDashboard.style.left = "0px";
        menu = 0;
    }
})
...
const mediaQueryBack = window.matchMedia('(max-width: 1000px)')

mediaQueryBack.addEventListener('change', (event) => {
    if (event.matches) {
        LDashboard.style.left = "-350px";
        RDashboard.style.left = "0px";
        menu = 0;
...
window.addEventListener("popstate", () => {
    ...
    const button = document.querySelector(`button[data-id="${link}"]`);
    button.click();
});
...
function coloredCode() {
    setTimeout(() => {
        /* PrismJS 1.29.0
        https://prismjs.com/download.html#themes=prism-okaidia&languages=markup+css+clike+javascript+aspnet+csharp */
